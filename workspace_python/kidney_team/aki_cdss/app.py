...
.pred-horizon { font-size: 0.75rem; color: #718096; font-weight: 600; letter-spacing: 1px; text-transform: uppercase; }
...
.patient-headerbar span { color: #718096; }
.patient-headerbar strong { color: #1a202c; }
...
/* 폼 입력 레이블 */
.form-label { font-size: 0.85rem; color: #4a5568; margin-bottom: 4px; }
...
.task-dot-done { width: 16px; height: 16px; border-radius: 50%; background: #e2e8f0; border: none; }
...
/* 임상 가이드라인 체크박스 */
.guide-row { display: flex; justify-content: space-between; align-items: center;
...
.guide-btn  { font-size: 0.72rem; color: #3182ce; border: 1px solid #90cdf4;
              background: #ebf8ff; border-radius: 4px; padding: 2px 10px; cursor: pointer; }
...
def generate_patients(n=20, seed=42):
    ...
    stages = [kdigo(delta_cr[i], urine_oi) for i, urine_oi in enumerate(urine_output)]

    risk_base = np.clip(
        delta_cr / base_cr * 0.45 + (1 / (urine_output + 0.1)) * 0.35 + bun / 250 * 0.2,
        0.05, 0.97
    )

    # 6h/12h/24h/48h 예측 확률 (시간이 길수록 약간 높게)
    ...
    return pd.DataFrame({
        ...
        "등록번호": reg_no,
        "병동": wards,
        "기저 Cr": base_cr.round(2),
        "현재 Cr": curr_cr.round(2),
        "ΔCr": delta_cr.round(2),
        "Urine Output": urine_output.round(2),
        "BUN": bun.round(1),
        "eGFR": egfr.round(1),
        "KDIGO": stages,
        "Risk": risk_base.round(3),
        "AKI_6h":  pred_6h.round(3),
        "AKI_12h": pred_12h.round(3),
        "AKI_24h": pred_24h.round(3),
        "AKI_48h": pred_48h.round(3),
    })
...
def generate_vitals(seed=0, hours=24):
    ...
    sbp = np.clip(rng.normal(125, 12, len(t)).cumsum() * 0 + rng.normal(125, 8, len(t)), 90, 165)
...
def generate_shap(seed=0):
    rng = np.random.default_rng(seed)
    features = [
        "현재 Creatinine", "ΔCr (48h)", "Urine Output",
        "BUN", "eGFR 변화율", "나이", "수분 섭취", "MAP",
        "CRP", "Bicarbonate",
    ]
...
with st.sidebar:
    ...
    for name, icon in pages.items():
        ...
        label_color = "#ffffff" if is_active else "#a0aec0"
        bg = "background: rgba(255,255,255,0.08); border-radius:8px;" if is_active else ""
...
if page == "홈":
    ...
    with col_top:
        ...
        <div style='text-align:right; padding-top:8px;'>
            <span style='font-size:0.85rem; color:#718096;'>{datetime.now().strftime('%Y년 %m월 %d일')}</span><br>
            <button style='background:#3182ce; color:white; border:none; border-radius:6px;
                           padding:4px 14px; font-size:0.82rem; cursor:pointer; margin-top:6px;'>회원가입</button>
        </div>
        """, unsafe_allow_html=True)
    ...
    for col, (label, num, badge, icon, color) in zip([sc1, sc2, sc3, sc4], stats):
        with col:
            badge_html = ""
            if badge == "FULL":
                badge_html = f"<span style='background:#fed7d7; color:#c53030; font-size:0.65rem; font-weight:700; padding:1px 6px; border-radius:4px; margin-left:6px;'>FULL</span>"
            elif badge == "주의":
                badge_html = f"<span style='background:#feebc8; color:#c05621; font-size:0.65rem; font-weight:700; padding:1px 6px; border-radius:4px; margin-left:6px;'>주의</span>"
            elif badge:
    ...
    with lk1:
        st.markdown("""
        <div class='shortcut-card' onclick="">
...
# ══════════════════════════════════════════════════════════
    ...
    col_form, col_right = st.columns([2, 3])
    with col_form:
        st.markdown("""
        <div class='card'>
                ...
                <tr style='border-bottom:1px solid #f0f0f0;'>
                    ...
                    <td style='padding:16px 0;'>
                        ...
                        <button style='margin-left:16px; background:white; border:1px solid #e2e8f0;
                                       ...
                                       cursor:pointer; color:#4a5568;'>수정 / 확인</button>
                    </td>
                </tr>
                <tr>
...
# ══════════════════════════════════════════════════════════
    ...
    <div style='background:white; border-bottom:1px solid #e2e8f0; padding:10px 0 10px;
        ...
        <span><span style='color:#718096; font-size:0.78rem;'>성별 </span>
              <strong style='font-size:0.85rem;'>{p['성별']}</strong></span>
    ...
    with col_main:
        ...
        with st.container():
            ...
            fig_v.add_trace(go.Scatter(x=vt, y=sbp, name="혈압(SBP)",
                                       line=dict(color="#3182ce", width=2), mode="lines+markers",
                                       marker=dict(size=4)))
            fig_v.add_trace(go.Scatter(x=vt, y=hr_v, name="심박수",
                                       line=dict(color="#e53e3e", width=2), mode="lines+markers",
                                       marker=dict(size=4)))
            fig_v.add_trace(go.Scatter(x=vt, y=temp * 3, name="체온(×3)",
                                       line=dict(color="#dd6b20", width=2, dash="dot"),
                                       mode="lines", opacity=0.7))
            fig_v.update_layout(
                height=200, paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="white",
                margin=dict(t=4, b=4, l=4, r=4),
                legend=dict(orientation="h", y=1.15, font=dict(size=11)),
                xaxis=dict(showgrid=True, gridcolor="#f0f0f0",
                           tickvals=[vt[0], vt[len(vt)//4], vt[len(vt)//2], vt[3*len(vt)//4], vt[-1]],
                           ticktext=[f"{h:02d}:00" for h in [0, 6, 12, 18, 24]][:5]),
        ...
        fig_pred.add_trace(go.Bar(
            x=horizons, y=[v * 100 for v in probs],
        ...
        def gauge(val, title, key):
            ...
            fig = go.Figure(go.Indicator(
                ...
                gauge={
                    ...
                    "borderwidth": 0,
                    "steps": [
                        {"range": [0, 40], "color": "rgba(56,161,105,0.1)"},
            ...
            fig.update_layout(height=160, paper_bgcolor="rgba(0,0,0,0)",
                              margin=dict(t=24, b=0, l=12, r=12))
            return fig

        with g1:
            st.plotly_chart(gauge(float(p["Risk"]), "종합 위험", "g_risk"),
                            use_container_width=True)
            kdigo = int(p["KDIGO"])
        ...
        with g2:
            st.plotly_chart(gauge(0.40, "낙상 위험", "g_fall"), use_container_width=True)
            st.markdown("<div style='text-align:center; margin-top:-8px; font-size:0.75rem; color:#718096;'>40%</div>", unsafe_allow_html=True)
        with g3:
            st.plotly_chart(gauge(0.20, "욕창 위험", "g_wound"), use_container_width=True)
            st.markdown("<div style='text-align:center; margin-top:-8px; font-size:0.75rem; color:#718096;'>20%</div>", unsafe_allow_html=True)
        st.markdown("</div>", unsafe_allow_html=True)
        ...
        fig_cr = go.Figure()
        fig_cr.add_hline(y=float(p["기저 Cr"]) * 1.5, line_dash="dash",
                         line_color="#dd6b20", line_width=1,
                         annotation_text="KDIGO 1 (×1.5)", annotation_font_color="#dd6b20",
                         annotation_position="right")
        fig_cr.add_hline(y=float(p["기저 Cr"]) * 2.0, line_dash="dash",
                         line_color="#e53e3e", line_width=1,
                         annotation_text="KDIGO 2 (×2.0)", annotation_font_color="#e53e3e",
                         annotation_position="right")
        fig_cr.add_vrect(x0=0, x1=50, fillcolor="rgba(59,130,246,0.04)", line_width=0)
        fig_cr.add_trace(go.Scatter(x=cr_t, y=cr_v, name="실측",
                                    line=dict(color="#3182ce", width=2),
                                    mode="lines+markers", marker=dict(size=5)))
        fig_cr.add_trace(go.Scatter(x=[cr_t[-1]] + fut_t, y=[cr_v[-1]] + fut_v,
                                    name="AI 예측",
                                    line=dict(color="#e53e3e", width=2, dash="dot"),
                                    mode="lines+markers",
                                    marker=dict(size=6, symbol="diamond", color="#e53e3e")))
        fig_cr.add_vline(x=0, line_dash="dash", line_color="#a0aec0", line_width=1,
                         annotation_text="현재", annotation_font_color="#a0aec0")
        fig_cr.update_layout(
            height=220, paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="white",
        ...
        fig_shap = go.Figure(go.Bar(
            x=sv, y=fs, orientation="h",
            marker_color=["#e53e3e" if v > 0 else "#3182ce" for v in sv],
            hovertemplate="%{y}: %{x:.3f}<extra></extra>",
        ))
        fig_shap.update_layout(
            height=260, paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="white",
            margin=dict(t=4, b=4, l=4, r=4),
    ...
    with col_right:
        ...
        st.markdown("<div class='sec-title' style='font-size:0.82rem; color:#718096; margin-bottom:8px;'>활성 알림</div>", unsafe_allow_html=True)
        ...
        if risk_v >= 0.7:
            ...
            <div class='alert-crit'>
                ...
                <div style='font-size:0.77rem; color:#742a2a; margin-top:4px;'>
                    처방된 와파린과 최근 추가된 이부프로펜 간의 잠재적 상호작용. 출혈 위험 증가.
                </div>
            </div>
            <div class='alert-warn'>
                ...
                <div style='font-size:0.77rem; color:#7b341e; margin-top:4px;'>
                    중복: 최근 48시간 이내 복부/골반 CT 스캔이 이미 처방됨.
                </div>
            </div>
            """, unsafe_allow_html=True)
        elif risk_v >= 0.4:
        ...
        st.markdown("<div class='sec-title' style='font-size:0.82rem; color:#718096; margin-bottom:8px;'>패혈증 관리 프로토콜</div>", unsafe_allow_html=True)
        guidelines = [
            "항생제 요법 시작 (권장: 피페라실린-타조박탐)",
            "젖산 수치 확인",
            "정맥 수액 투여",
        ]
        for g in guidelines:
        ...
        st.markdown("<br>", unsafe_allow_html=True)
        st.markdown("<div class='sec-title' style='font-size:0.82rem; color:#718096; margin-bottom:8px;'>주요 추세 분석</div>", unsafe_allow_html=True)
        # 작은 추세 차트
        x2 = list(range(31))
        y_wbc = np.clip(np.cumsum(np.random.default_rng(sel_idx).normal(0.1, 0.5, 31)) + 10, 4, 22)
        y_cr2 = np.clip(np.cumsum(np.random.default_rng(sel_idx+10).normal(0.03, 0.08, 31)) + 0.8, 0.3, 4)
        fig_sm = go.Figure()
        fig_sm.add_trace(go.Scatter(x=x2, y=y_wbc, name="WBC", line=dict(color="#e53e3e", width=1.5), yaxis="y"))
        fig_sm.add_trace(go.Scatter(x=x2, y=y_cr2, name="Cr", line=dict(color="#3182ce", width=1.5), yaxis="y2"))
        fig_sm.update_layout(
            height=150, paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="white",
            margin=dict(t=4, b=4, l=4, r=32),
            xaxis=dict(showgrid=True, gridcolor="#f0f0f0", tickfont=dict(size=9)),
            yaxis=dict(title="WBC", showgrid=True, gridcolor="#f0f0f0",
                       titlefont=dict(color="#e53e3e"), tickfont=dict(size=9, color="#e53e3e")),
            yaxis2=dict(title="Cr", overlaying="y", side="right",
                        titlefont=dict(color="#3182ce"), tickfont=dict(size=9, color="#3182ce")),
        ...
        st.markdown("<div class='sec-title' style='font-size:0.87rem;'>AI 분석 보고서</div>", unsafe_allow_html=True)
        st.markdown(f"""
        <div style='font-size:0.82rem; color:#4a5568; line-height:1.6; background:#f7fafc;
                    border-radius:8px; padding:12px; margin-bottom:12px;'>
            최근 백혈구 수치 증가와 발열 스파이크는 초기 패혈증을 강력히 시사합니다.
            배양 결과로 확인하십시오.<br><br>
            현재 환자 AKI 48h 예측: <strong style='color:{risk_color(float(p["AKI_48h"]))}'>{float(p["AKI_48h"])*100:.1f}%</strong> —
            {'즉각적인 신장 보호 중재 권장.' if float(p["AKI_48h"]) >= 0.7 else '4~6시간 후 재평가 권장.'}
        </div>
        """, unsafe_allow_html=True)
        st.markdown("<div class='sec-title' style='font-size:0.87rem;'>관련 참고 문헌</div>", unsafe_allow_html=True)
        st.markdown("""
...
# ══════════════════════════════════════════════════════════
    ...
    with main_col:
            ...
            with top_b[0]:
                ...
                <div class='card'>
                    ...
                    <div class='step-wrap'>
                        ...
                        <div class='step'>
                            <div class='step-circle step-idle'>2</div>
                            <div class='step-label'>서류 제출</div>
                        </div>
                        <div class='step-line'></div>
                        <div class='step'>
                            <div class='step-circle step-idle'>3</div>
                            <div class='step-label'>최종 확인</div>
                        </div>
                        <div class='step-line'></div>
                        <div class='step'>
                            <div class='step-circle step-idle'>4</div>
                            <div class='step-label'>등록 완료</div>
                        </div>
                    </div>
                </div>
                """, unsafe_allow_html=True)
            ...
            with top_b[1]:
                ...
                <div class='card' style='height:100%;'>
                    ...
                    <div style='font-size:0.8rem; color:#4a5568; line-height:1.8;
                        ...
                        <p>* 최근 방문: 2026.04.10 정형외과 외래 (접수자: 김민호)</p>
                        <p>* 행정: 개인정보 수집 동의서 갱신 필요 (유효기간: 2026.04.09 만료)</p>
                        <p>* 안내: 장기 미방문 환자로 신규 등록 프로세스 일부 적용</p>
        ...
        with bot3_r:
            st.markdown("""
            <div class='card'>
                <div class='sec-title'>빠른 등록 기능</div>
            ...
            <div class='card' style='margin-top:0;'>
                ...
                <div style='display:flex; justify-content:space-between; align-items:center; padding:8px 0; border-bottom:1px solid #f7fafc;'>
                    ...
                    <span style='background:#fed7d7; color:#c53030; padding:1px 8px; border-radius:10px;
                                 font-size:0.75rem; font-weight:700;'>2</span>
                </div>
...
# ══════════════════════════════════════════════════════════
    ...
    st.markdown("<div class='card'><p style='color:#718096; text-align:center; padding:40px;'>일정 기능은 준비 중입니다.</p></div>", unsafe_allow_html=True)
